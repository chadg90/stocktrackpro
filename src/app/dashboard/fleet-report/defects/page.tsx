...
export default function FleetReportDefectsPage() {
  ...
  const row = (d: (typeof defects)[0], key: string) => (
    <tr key={key} className="border-b border-white/5">
      ...
      <td className="px-3 py-2 text-white/70 whitespace-nowrap max-w-[120px] truncate" title={d.reporter_contact_phone || ''}>
        {d.reporter_contact_phone || '—'}
      </td>
          ...
          <table className="w-full text-sm min-w-[820px]">
            ...
            <tbody>
              {loading && outstandingDefects.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-3 py-6 text-white/50">
                    Loading…
                  </td>
                </tr>
