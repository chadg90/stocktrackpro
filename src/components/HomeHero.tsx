...
export default function HomeHero() {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col justify-center px-5 sm:px-6 lg:px-10 xl:px-16 pt-8 pb-10 sm:py-16 lg:py-24 order-1 lg:order-1 text-center lg:text-left items-center lg:items-start">
        <p className="text-[var(--brand-blue)] font-medium text-xs sm:text-sm uppercase tracking-[0.18em] sm:tracking-[0.2em] mb-3 sm:mb-5">
          UK DVSA van fleet compliance platform
        </p>
        <h1 className="text-[1.875rem] sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold tracking-tight text-white mb-4 sm:mb-6 leading-[1.12] sm:leading-[1.08] max-w-xl lg:max-w-none">
          Van fleet management, walkaround checks &amp; defect resolution
        </h1>
        <p className="text-[0.95rem] sm:text-lg md:text-xl text-white/80 max-w-md sm:max-w-2xl leading-relaxed mb-7 sm:mb-10">
          Daily DVSA walkaround checks, instant defect close-out, and live MOT visibility — mobile app for drivers in
          the yard, web dashboard for transport managers. Optional Plant &amp; Machinery for LOLER site records.
        </p>
        <div className="flex w-full max-w-sm sm:max-w-none flex-col sm:flex-row gap-3 sm:gap-4 lg:w-auto">
          ...
          <a
            ...
            className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-6 py-3.5 sm:py-4 min-h-[48px] rounded-xl border border-white/20 text-white font-medium bg-white/5 hover:bg-white/10 transition-colors"
          >
      ...
      <div className="relative order-2 lg:order-2 w-full lg:h-full lg:min-h-[calc(100vh-6rem)]">
        <div className="relative mx-4 mt-2 h-[30vh] min-h-[220px] max-h-[300px] overflow-hidden rounded-2xl border border-white/10 sm:mx-6 sm:mt-0 sm:h-auto sm:max-h-none sm:aspect-[4/5] sm:rounded-none sm:border-0 lg:mx-0 lg:aspect-auto lg:h-full lg:rounded-none">
          {showVideo ? (
            <video
              ref={videoRef}
